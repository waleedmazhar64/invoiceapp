...
if (isset($jsondata['InvoiceIDS'])) 
{
    $InvoiceIDS = $jsondata['InvoiceIDS']; //$conn->insert_id
	$InvoiceIDS = json_encode($InvoiceIDS);
 ...
 include 'database.php';
    ...
    $con = mysqli_connect($db_host, $db_username, $db_password,$db_name) or die(mysql_error());
         $someObject = json_decode($InvoiceIDS);
		 foreach($someObject as $value) {
              $result1 = mysqli_query($con,"DELETE from invoiceitems where Iid='$value->ID'");
	        //  $result2 = mysqli_query($con,"DELETE from payments where Iid='$invoiceId'");
	          $result2 = mysqli_query($con,"DELETE from invoices where Iid='$value->ID'");
