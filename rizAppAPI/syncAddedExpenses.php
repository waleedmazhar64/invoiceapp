...
if (isset($jsondata['Expenses'])) 
{
    $Expenses = $jsondata['Expenses']; //$conn->insert_id
	$Expenses = json_encode($Expenses);
 
    // mysql inserting a new row
	 
         $con = mysqli_connect($db_host, $db_username, $db_password,$db_name) or die(mysql_error());
         $someObject = json_decode($Expenses);
		 foreach($someObject as $value) {

		     

			 $sql1 = "INSERT INTO tblexpenses (ID,AccountId,Name,Date,Amount,Category,Description,Active,Sync) VALUES ('$value->ID','$value->AccountId','$value->Name','$value->Date','$value->Amount','$value->Category','$value->Description','$value->Active','$value->Sync')";
