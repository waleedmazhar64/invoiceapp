...
if (isset($jsondata['POSItems'])) 
{
    $POSItems = $jsondata['POSItems']; //$conn->insert_id
	$POSItems = json_encode($POSItems);
 
    // mysql inserting a new row
	 
         $con = mysqli_connect($db_host, $db_username, $db_password,$db_name) or die(mysql_error());

         $someObject = json_decode($POSItems);
		 foreach($someObject as $value) {

			 
			 $sql1 = "INSERT INTO tblpodetail(ID,Poid,ItemID,Qty,CostPrice) VALUES ('$value->ID','$value->Poid','$value->ItemID','$value->Qty','$value->CostPrice')";
